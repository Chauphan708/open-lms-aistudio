...
import { Swords, Clock, Zap, Shield, CheckCircle, XCircle } from 'lucide-react';
...
export const PvPBattle: React.FC = () => {
    ...
    const { user, arenaProfile, submitArenaAnswer, updateMatchHp, finishMatch, users } = useStore();
    ...
    const opponentAvatar = useRef('warrior');
        ...
        return (
            <div className="min-h-[60vh] flex items-center justify-center">
                <div className="text-center">
                    <Swords className="h-12 w-12 text-red-500 mx-auto mb-4" style={{ animation: 'pulse 1s ease-in-out infinite' }} />
    ...
    return (
        ...
        @keyframes slash { 0% { transform: scale(0) rotate(-45deg); opacity: 1; } 50% { transform: scale(1.5) rotate(-45deg); opacity: 0.5; } 100% { transform: scale(2) rotate(-45deg); opacity: 0; } }
      ...
      `}</style>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6 items-center" style={{ animation: 'fadeIn 0.5s ease-out' }}>
                ...
                {/* Opponent Avatar */}
                <div className={`text-center p-4 rounded-2xl border-2 transition-all ${opDamageAnim ? 'border-red-500' : 'border-red-100'}`}
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border p-6 mb-4" style={{ animation: 'fadeIn 0.4s ease-out' }}>
                        <div className="flex items-center gap-2 mb-3">
                            <span className="bg-indigo-100 text-indigo-700 px-2.5 py-0.5 rounded-full text-xs font-bold">
                                {currentQuestion.subject === 'math' ? '📐 Toán' : currentQuestion.subject === 'science' ? '🔬 Khoa học' : '💻 Công nghệ'}
                            </span>
                        </div>
